...
class SegmentWithoutQRs:
    ...
    def make_reduce(self, reduce_method=cv2.REDUCE_AVG, filtered=True, filter_coef=0.33):
        ...
        if filtered:
            ...
            ll_average = int(
                np.average(np.sort(reduced_horizontal, axis=0)[-int(self._pack_height_px * self.rows_amount):]))
            too_small = reduced_horizontal[:, 0] < ll_average * filter_coef
            reduced_horizontal[too_small] = 0

        return reduced_horizontal

    def create_regions(self, array, filtered=True):
        pt1, pt2 = 0, 0
        found = False
        regions = []

        for index in range(array.shape[0]):
            if array[index, 0] and not found:
                pt1, found = index, True
            if index == array.shape[0] - 1 and found:
                pt2, found = index, False
                regions.append([pt1, pt2])
            elif not array[index + 1, 0] and found:
                pt2, found = index, False
                regions.append([pt1, pt2])
            elif index == array.shape[0] - 1 and not found:
                break
        ...
        if filtered:
            ...
            regions = regions[sizes(regions) > self._interval_px]

        return regions

    def low_height(self, array, hight_coef=0.8):
        return array[1] - array[0] < self._pack_height_px * hight_coef

    def low_interval(self, array, index, interval_coef=1.5):
        return array[index + 1, 0] - array[index, 1] < self._interval_px * interval_coef

    def connect_regions(self, array, index=0):
        if index < array.shape[0] - 1:
            if self.low_height(array[index, :]) and self.low_height(array[index + 1, :]):
                if self.low_interval(array, index):
    ...
    def visualise(self, array, segment=True):
        if array.shape[0] == self.__image_height:
...
x = SegmentWithoutQRs('test6.jpg', rows_amount=7)
reduced = x.make_reduce()
regions = x.create_regions(reduced)
# x.connect_regions(regions)
